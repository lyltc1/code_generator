...
opt.background_color = np.asarray([0.5, 0.0, 0.0])
...
if SAVE_IMAGES:
    ...
    for i in range(0, 16):
        ...
        shifted_value_1 = np.right_shift(vertex_id, (iteration - i - 1)).astype(float)
        colors[:] = (shifted_value_1 / float(class_base ** (i + 1) - 1))[:, None]
        ...
        if SAVE_IMAGES:
            vis.poll_events()
            vis.update_renderer()
            saved_image_path = join(project_path, "output", f"v2_{i}.png")
            vis.capture_screen_image(saved_image_path)
