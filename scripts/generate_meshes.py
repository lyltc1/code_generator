...
def get_model_params(data_dir, dataset_name):
  """Returns parameters of object models for the specified dataset.

  :param data_dir: Path to a folder with datasets.
  ...
  bop_dir = os.path.join(data_dir, 'bop')

  p = {
    # ID's of all objects included in the dataset.
    'obj_ids': obj_ids,
    # Path template to an object model file.
    'ply_tpath': os.path.join(data_dir, 'remesh_visible', dataset_name, 'obj_{obj_id:06d}.ply'),

    # Path to a file with meta information about the object models.
    'models_info_path': os.path.join(bop_dir, dataset_name, 'models', 'models_info.json'),
...
class DatasetInfo:
  def __init__(self, p):
    ...
    dp_model = get_model_params(p['data_dir'], p['dataset'])
...
class DividedPcd:
  ...
  def pair_continuous_sym_pcd(self, threshold=0.0001):
    ...
    while pairs_to_be_divided:
      print(f"pairs to be divided is {len(pairs_to_be_divided)}")
  ...
  def map_coordinates_each_pair_to_same(self, coordinate_shift=False):
    """ map points coordinates of each pair to same one
        output: map_coordinate: ndarray, shape = [len(self.origin_pcd.points), 3]
    """
    ...
    # ---- construct map_coordinate ----
    map_coordinate = np.empty([len(self.origin_pcd.points), 3])
    points = np.array(self.origin_pcd.points)
    for sym_type, pairs in self.pairs.items():
      for pair in pairs:
        local_index = np.argmax(np.sum(points[pair], axis=1))
        select_index = pair[local_index]
        coordinate = points[select_index]
        if sym_type == 'continuous sym':
          ...
          if coordinate_shift is True:
            coordinate[axis_keep] = coordinate[axis_keep] + 1.1 * self.model_info[axis_map[axis_keep]]
        map_coordinate[pair] = coordinate
    return map_coordinate

  def divide_pointcloud_iterative(self, map_coordinates, number_of_iteration=16, divide_number=2):
    ...
    num_points = len(map_coordinates)
  ...
  def _divide_cluster(self, input_indices, coordinates, divide_number):
    ...
    if num_points == 0:
      return [[] for i in range(divide_number)]
    if (coordinates[input_indices] == coordinates[input_indices][0]).all():
      # In this case, all coordinates is the same, so we needn't to divide
      output_indices = []
      output_indices.append(input_indices)
      for i in range(divide_number - 1):
        output_indices.append([])
      return output_indices
    kmeans = KMeans(n_clusters=divide_number, random_state=0)
    kmeans.fit_predict(coordinates[input_indices])
...
class AppWindow:
  ...
  def _on_buttion_save_result(self):
    ...
    json_path = os.path.join(self.dataset_info.save_path,
                             'obj_{obj_id:06d}.json'.format(obj_id=result['obj_id']))
  ...
  def _on_buttion_load_result(self):
    ...
    if obj_id == 0:
      ...
      return
    json_path = os.path.join(self.dataset_info.save_path,
                             'obj_{obj_id:06d}.json'.format(obj_id=obj_id))
    if os.path.exists(json_path):
  ...
  def _on_button_divide_iter(self):
    ...
    # if coordinate_shift = False, then the coordinate will not change
    map_coordinate = self.divided_pcd.map_coordinates_each_pair_to_same(coordinate_shift=False)
    one_point_each_pair_pcd_non_shift = o3d.io.read_point_cloud("None")
    one_point_each_pair_pcd_non_shift.points = o3d.utility.Vector3dVector(map_coordinate)
    colors = one_point_each_pair_pcd_non_shift.points
  ...
  def _on_button_save_final(self):
    ...
    #                                'obj_{obj_id:06d}_additional_points.ply'.format(obj_id=self.divided_pcd.obj_id))
    # if not os.path.exists(self.dataset_info.save_path):
    #   os.makedirs(self.dataset_info.save_path)
    # o3d.io.write_triangle_mesh(save_model_path, self.divided_pcd.labeled_mesh_additional_points, write_ascii=True)
    # save model_mesh_v2, (without additional points)
    save_model_path = os.path.join(self.dataset_info.save_path,
    ...
    with open(save_corresponding_path, 'w') as f:
      f.write(json.dumps(result))
    print(f"save file: {save_corresponding_path}")
def main():
